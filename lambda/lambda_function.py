...
from config import (DASHBOARD_URL, MOBILE_MODE, DEBUG)
...
def open_page(number):
    url = create_presigned_url('amzn-s3-alexa-premium-bucket')
    if number is None or number == "":
        number = 1
    if MOBILE_MODE: 
        source = DASHBOARD_URL + "&object_id=" + str(number) + "&mobile=1"
    else:
        source = DASHBOARD_URL + "&object_id=" + str(number) + "&token=" + str(TOKEN) + "&presigned_url=" + str(url)
    ...
    def handle(self, handler_input):
        ...
        response_builder.add_directive(
            RenderDocumentDirective(
                token=TOKEN,
                document=_load_apl_document("template3.json")
            )
        )
...
class OpenPageIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        logger.info("OpenPageIntentHandler")
        ...
        response_builder.add_directive(
            RenderDocumentDirective(
                token=TOKEN,
                document=_load_apl_document("template2.json")
            )
        )
...
class IntentReflectorHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        logger.info("IntentReflectorHandler")
        return (
            handler_input.response_builder
                .speak(speak_output)
